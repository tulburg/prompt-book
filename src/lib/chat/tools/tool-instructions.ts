...
function buildContextPolicyLines(tools: ChatToolDefinition[]): string[] {
  ...
  return [
    ...
    '- CRITICAL: From the listed context, y you must load the relevant context with `Context` and `action: "read"` before proceeding - the relevant context for the changes you are about to make. The context will contain all information you will need.',
    ...
    "- CRITICAL: Example of a good feature context pointer: `User login feature accepts emails and password: src/auth/login.ts:20-25`",
    "- CRITICAL: Example of a good file context pointer: `All api request goes through the useApiHook: src/hooks/useApi.ts`",
    "- CRITICAL: Example of a good property context pointer: `Codebase package manager is pnpm@v7: node_modules/.bin/pnpm`",
    ...
    "- A context is a point-by-point pointer to the project files, functions, line number. Context is direction map of the codebase.",
