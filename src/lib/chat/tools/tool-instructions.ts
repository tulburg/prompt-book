...
function buildContextPolicyLines(tools: ChatToolDefinition[]): string[] {
  ...
  return [
    ...
    '- If the list is empty or no existing context fits the current change, create one with `Context` and `action: "write"`, including a markdown filename like `codebase.md`, a title, a description, and a `content_body`, then read it before continuing.',
...
function buildBlockPolicyLines(tools: ChatToolDefinition[]): string[] {
  ...
  return [
    ...
    "- Updating a block means keeping its title, definition, files list, linked context, and diagram current. The linked `context_body` is a full replacement of point-by-point pointers, not an appended log entry. Update the diagram when the block flow changed.",
